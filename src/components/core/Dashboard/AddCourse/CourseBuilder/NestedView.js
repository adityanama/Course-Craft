...
const NestedView = ({ handleChangeEdit }) => {
    ...
    return (
        <>
                    ...
                    course.courseContent.map((section) => (
                        <details key={section._id} open>
                            <summary className="flex cursor-pointer items-center justify-between border-b-2 border-b-richblack-600 py-2">
                                ...
                                <div className='flex items-center gap-x-3'>
                                    <button onClick={() => handleChangeEdit(section._id, section.sectionName)}>
                                        <MdEdit className='text-xl text-rich' />
                                    </button>

                                    <button onClick={() => setConfModal({
                                    ...
                                    <span className='font-medium text-richblack-300' ><AiFillCaretDown className={`text-xl text-richblack-300`} /></span>
                                </div>
                            </summary>
                                    ...
                                    section.subSection.map((data) => (
                                        <div className="flex cursor-pointer items-center justify-between gap-x-3 border-b-2 border-b-richblack-600 py-2" key={data._id} onClick={() => setViewSubSection(data)}>
                                            <div className="flex items-center gap-x-3 py-2 ">
                                                <FaCirclePlay className='text-xl text-richblack-300' />
            ...
            {confModal ? (
                <ConfirmationModal modalData={confModal} />
            ) : (
                <></>
            )}
        </>
